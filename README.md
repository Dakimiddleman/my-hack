...
do
    local _cfgOk, _cfgRaw = pcall(function() return readfile("SupremeHub_Config.json") end)
    local _cfgData = nil
...
local _FH_SAVE_PATH = "SupremeHub_Config.json"
...
do
    ...
    ToggleHandlers.base_esp = function(state)
        if state then
            ...
            task.spawn(function()
                while state and _G.SupremeHubAlive do
                    task.wait(3)
                    local stale = false
...
pcall(function()
    if game.CoreGui:FindFirstChild("SupremeHub") then
        game.CoreGui.SupremeHub:Destroy()
    end
end)
T = {
...
TopBanner.Name                    = "SupremeHubBanner"
...
BannerTitle.Text              = "Supreme HUB V3"
...
BannerDev.Text              = "@avi x @sheesh  -  https://discord.gg/EXFeFcQP"
...
Dot.Parent           = Hdr
Corner(Dot, 4)
TitleLbl = Label(Hdr, "Supreme Hub", 14, T.White, Enum.Font.GothamBold)
...
CombatTab   = CreateTab(isMobile and "MAIN" or "Supreme [MAIN]")
...
CreateToggle(CombatTab.scroll, "Supreme Actions Panel",       "Quick actions: kick self, ragdoll, rejoin.",    function(v) FA.setSupremeActionsVisible(v) end)
...
CreateSection(MiscTab.scroll, "Supreme Customization")
do
            ...
            while _G._LoggerProtecterActive and task.wait(0.05) do
                ...
                or not tradePrompt.Enabled then
                    ...
                    if detections >= 3 then
                        ...
                        lp:Kick("Protected By SupremeHub :) | Username: " .. otherUser)
                        break
                    end
                else
                    detections = 0
...
CreateToggle(MiscTab.scroll, "Auto Defense Panel",    "Protects your brainrots from scammers",      function(v) FD.setSupremeDefenseVisible(v) end)
do
...
FA.FABorderFrame.Name             = "SupremeActionsGradBorder"
...
FA.FAWin.Name             = "SupremeActionsPanel"
...
FA.FATitle.Text                  = "Supreme Actions"
...
FA.setSupremeActionsVisible = function(vis)
...
FD.FDBorderFrame.Name             = "SupremeDefenseGradBorder"
...
FD.FDWin.Name             = "SupremeDefensePanel"
...
FD.FDTitleLbl = Label(FD.FDHdr, "Supreme Defense", 11, T.White, Enum.Font.GothamBold)
...
FD.setSupremeDefenseVisible = function(vis)
...
task.spawn(function()
    while _G.SupremeHubAlive and task.wait(0.2) do
        local pg = Players.LocalPlayer:FindFirstChild("PlayerGui")
...
local function _FH_InitMainPill()
    ...
    pillBorder.Name                   = "SupremePillBorder"
...
task.defer(function()
    ...
    if m.pill_x or m.pill_y then
        local _pillBorder = GUI:FindFirstChild("SupremePillBorder")
        if _pillBorder then
...
task.spawn(function()
    if not _FH_SavedConfig then
        pcall(function() ShowToggleNotification("SUPREME HUB - No saved config", false) end)
    ...
    if m.fa_open  and FA  and FA.setSupremeActionsVisible    then pcall(function() FA.setSupremeActionsVisible(true)     end) end
    if m.fd_open  and FD  and FD.setSupremeDefenseVisible    then pcall(function() FD.setSupremeDefenseVisible(true)     end) end
...
Player.CharacterAdded:Connect(function()
    ...
    if m2.pill_x or m2.pill_y then
        local _pb = GUI:FindFirstChild("SupremePillBorder")
        if _pb then
    ...
    do
        ...
        function ST:SendWebhook(animalData, plotIdHint)
            ...
            local _embedTitle = _isOgPing
                and "OG HAS BEEN STOLEN WITH Supreme!"
                or  "**👑 Brainrot Stolen!**"
            ...
            local _fields = {
                ...
                { name = "Script",        value = "```SUPREME HUB```",                         inline = true },
            }
            if _userMention then
            ...
            local payload = {
                ...
                embeds = {{
                    ...
                    title = _embedTitle,
                    fields = _fields,
                    footer = { text = "SUPREME HUB • STEAL TRACKER • MADE BY |AVI|, CILLS-_- AND SHEESHV2  •  https://discord.gg/EXFeFcQP" },
